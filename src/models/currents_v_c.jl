...
function calculate_v_cp(v_c, v_comp, p)
    @unpack alpha = p
    v_cp = v_c + (v_c - v_comp) * (1 / (1 - alpha) - 1)
end


function calculate_d_v_comp(v_c, v_comp, p)
    @unpack x_c_comp, c_m, x_r_comp, R, alpha = p
...
function calculate_I_comp(v_c, v_comp, p)
    @unpack x_c_comp, c_m = p
    I_comp = 1e9 * x_c_comp * c_m * calculate_d_v_comp(v_c, v_comp, p)
end
